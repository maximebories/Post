#!/usr/bin/env bash

defaults write com.apple.amsaccountsd "CKPerBootTasks" '
<array>
	<string>CKAcccountInfoCacheReset</string>
</array>
'
defaults write com.apple.amsaccountsd "CKStartupTime" -integer 1633285015
defaults write com.apple.amsaccountsd "CloudKitAccountInfoCache" '
<dict>
	<key>6ff43430fd7b8b7b406b38ee7c3e717c47c1a2986500ff2f4a9828640a460026</key>
	<data>
	
	AAAAAAIBAAAAAAAAAB4AAAAAAAAAAAAAAAAAAAEq
	</data>
</dict>
'
